...
export async function generatePreciseWorkflow(query: string, context?: string): Promise<PreciseWorkflow> {
  ...
  try {
    const { text } = await generateText({
      ...
      system: `You are an expert workflow architect and efficiency consultant. Your task is to create precise, step-by-step workflows that maximize efficiency and minimize wasted effort.
